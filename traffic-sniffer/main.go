...
func (p *Proxy) writeResponse(w http.ResponseWriter, res *http.Response) {
	...
	fmt.Printf("XXXXXXX header server value before set %s\n", w.Header().Get("server"))

	// Set a special header to notify that the proxy actually serviced the request.
	// this never gets set, the return value gets overwritten by the value of `envoy`
	w.Header().Set("server", "amazing-proxy")

	fmt.Printf("YYYYYYY header server value after set %s\n", w.Header().Get("server"))
